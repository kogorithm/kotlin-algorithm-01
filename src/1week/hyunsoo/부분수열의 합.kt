class `부분수열의 합` {
    companion object {
        fun getThis() = `부분수열의 합`()
    }

    val sizeAndCnt = readln().split(" ").map { it.toInt() }
    val num = readln().split(" ").map { it.toInt() }
    val result = mutableListOf<Int>()
    var ans1 = 0

    public fun combination(cnt: Int, depth: Int, beginwith: Int) {
        if (cnt == depth) {
            if (result.sum() == sizeAndCnt[1]) ans1++
            return
        }
        for (cur in beginwith until num.size) {
            result.add(num[cur])
            combination(cnt + 1, depth, cur + 1)
            result.removeAt(result.lastIndex)
        }
    }

}
fun main() {
    val temp = `부분수열의 합`.getThis()
    for (depth in 0 .. temp.num.size){
        temp.combination(0, depth, 0)
    }

    if(temp.sizeAndCnt[1] == 0) temp.ans1 -= 1
    println(temp.ans1)

}