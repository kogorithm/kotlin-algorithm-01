...
fun <T> List<T>.combination1(num: Int): List<List<T>> {
    if (num == 1) {
    ...
    for (i in 0 until length) {
        ...
        val combinations = this.subList(i + 1, length).combination1(num - 1)
...
fun String.parseIntPair1(): Pair<Int, Int> {
    val (a, b) = split(" ")
...
fun main() {
    ...
    val tripleList = IntRange(1, N).map {
        val (range, price) = br.readLine().parseIntPair1()
        Triple(it, range, price)
    }

    val answer = IntRange(1, N).maxOf {
        tripleList.combination1(it).fold<List<Triple<Int, Int, Int>>, MutableList<Int>>(mutableListOf()) { prev, next ->
